...
## 本地备份
每次创建一个新的笔记，inBox 都会自动打包所有的笔记到一个文件中，并对数据进行加密，然后存储在本地文件夹下，目录为：`/Android/data/name.gudong.think/files/Documents/inBox/Backups/Auto`，这样做的目的是防止应用被意外卸载后，数据丢失。

因为默认数据都存储在 APP 的文件沙盒中，该文件会随着 APP 卸载被删除，所以这里通过该机制，将数据备份至外部可访问目录，防止笔记数据丢失，同时为了防止明文数据泄露，这里对数据进行了 AES 加密。

当下次重新安装 inBox 笔记软件时，inBox 会自动检查备份文件夹，并提示用户进行数据恢复。
