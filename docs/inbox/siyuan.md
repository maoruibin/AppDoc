...
把 inBox 云端的笔记**单向同步**到 [思源笔记](https://b3log.org/siyuan/) 的第三方插件。
...
1. 打开思源 → **设置 → 集市**
2. 在搜索框输入 **inBox** 或 **inBox 同步**
3. 找到 **inBox 同步** 插件，点 **下载** 安装
4. 在 **设置 → 饱和插件**（或插件管理）里启用 **inBox 同步**

> 集市版本可能落后 GitHub 主线几天到几周，求新可以走方式 2。

### 方式 2：手动安装（从 GitHub Releases）

1. 从 [Releases](https://github.com/maoruibin/siyuan-inbox-sync/releases) 下载最新的 `package.zip`
...
inBox 的定位是**收集箱**,所有笔记/批注/盒子都在 App 端创建。思源这边当作**只读工作台**:能完整拉历史、按盒子分文件夹、查标签,但不把思源里的编辑/删除回传到云端。
